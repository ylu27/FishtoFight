...
*! Author: Roger Newson
*! Date: 09 June 2003     (SJ3-3: st0043)
*/

syntax [ anything(name=gaplist id="gap list") ] [if] [in] [ , Gapindicator(string) AFter
  ROwlabel(varlist string) GRowlabels(string asis) RString(string) ];
if `"`gaplist'"'=="" {;
  local gaplist=1;
};
else {;
...
qui gene byte `gapindicator'=0 if `touse';
...
* Set non-by, non-rowlabel variables in by-gap observations *;
if `"`existvar'"'!="" {;
...
* Restore old data only if user presses -Break- *;
restore,not;
